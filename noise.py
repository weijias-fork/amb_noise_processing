...
#            picks.append([freq,closest_vel])
        ...
        # QUALITY CHECKING OF PREVIOUS PICKS
        if len(picks)>0:
        ...
        # STARTS A FEW CYCLES BEHIND THE ZERO CROSSING SMOOTHING
            ...
            if len(picks)==0:
                ...
                for j in range(np.int(three_freq_step/2)):
                    ...
                    if idxmax[idxpick]==idxmax[-1] or idxmax[idxpick]==idxmax[0]: #there should be one faster and one slower maximum nearby.
                        #if plotting:
                         #   print "%.2f %.2f" %(X[0][-three_freq_step+j],yaxis[idxmax[idxpick]]),"discarding pick: no other maxima nearby"
                        continue
    ...
    # PICKING TO THE END OF THE WINDOW
